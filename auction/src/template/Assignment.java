...
public class Assignment {
	...
	/** the index of a Action in a route, the same as the times array in the project description*/
	public final Map<Action, Integer> indexOf;
	private final ObjFunc costFunction = new ObjFunc();
	public Long cost;
	
	/** a flag to tell if this assigment is corrupt (true -> does not violate any constraint) */
	private boolean mNotCorrupt = true;
	
	private Random rand = new Random(/* 2015 */);
	
	public Assignment(	Map<Vehicle, List<Action>> vehicleRoutes, 
						Map<Task, Vehicle> vehicles,
	...
	public Assignment(List<Vehicle> vehicles) {
		
		this.vehicleRoutes = new HashMap<Vehicle, List<Action>>();
		this.vehicles = new HashMap<Task, Vehicle>();
		this.indexOf = new HashMap<Action, Integer>();
	...
	public List<Plan> generatePlans(List<Vehicle> vehicles) {
		...
		for (Vehicle vehicle : vehicles) {
			...
			if (!vehicleRoutes.get(vehicle).isEmpty()) {
				// the vehicle has at least one action to do (actually two since it has
				// at least to pickup and deliver a package)
				City currentCity = vehicle.getCurrentCity();
				for (Action nextAction : vehicleRoutes.get(vehicle)) {
					for (City miniGoal : currentCity.pathTo(nextAction.actionCity)) {
						plan.appendMove(miniGoal);
					}
					currentCity = nextAction.actionCity;
					
					if (nextAction.isDelivery()) {
						plan.appendDelivery(nextAction.task);
					} else if (nextAction.isPickup()) {
	...
	public TreeSet<Assignment> generateAllNeighbors(
		...
		for (Task task : tasks) {
			...
			Action action = rand.nextBoolean() ? new Pickup(task) : new Deliver(task);
			
			// move
			List<Action> route = vehicleRoutes.get(vehicles.get(task));
			int maxdir = route.size();
			
			for (int i = -maxdir; i <= maxdir; i++) {
				assignment = this.copy();
				if (assignment.moveAction(action, i)) {
					if (nabos.add(assignment)) {
			...
			for (Vehicle v : vehicleRoutes.keySet()) {
				assignment = this.copy();
				if (assignment.changeVehicle(v, task)) {
					nabos.add(assignment);
				}
			}
		}
	...
	private boolean moveAction(Action act, int distance) {
		...
		if (!Constraints.checkPickupBeforeDeliveryConstraint(
		    this)) { throw new IllegalStateException("moveAction not correct from the start!"); }
		...
		if (route.size()
		    % 2 != 0) { throw new IllegalStateException("A route must have a even size (always one pickup and delivery)"); }
		...
		if (!Constraints
		    .checkPickupBeforeDeliveryConstraint(this)) { throw new IllegalStateException("moveAction not correct yet!"); }
		return mNotCorrupt;
