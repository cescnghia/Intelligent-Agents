...
public class PickupDeliveryProblem {
	...
	public A SelectInitialSolution_2(){
		...
		for (Task t : mTasks){
			...
			LinkedList<Task_> actuallyList = map.get(v);
			actuallyList.add(new Task_(t, Action.DELIVERY));
			actuallyList.add(new Task_(t, Action.PICKUP));
			map.put(v, actuallyList);
		}
		...
		for (int i = 0; i < iter ; i++){
			...
			for (A a : plans)
				if (!checkConstraint(a))
					System.out.println("What the hoi");
